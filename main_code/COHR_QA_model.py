"""
COHR (Coherent) 季報財務資料：以 yfinance 抓取、輸出 CSV、上傳 Google Sheets。
執行目錄不限；credentials 預設為專案根目錄的 credentials.json。

目標試算表（COHR_QA_FM）：
https://docs.google.com/spreadsheets/d/1wg7FLeNjZlauarRrsoB6mfG1NENq3157XlZTXv2Y1Y4/edit
...
TICKER = "COHR"
SPREADSHEET_ID = "1wg7FLeNjZlauarRrsoB6mfG1NENq3157XlZTXv2Y1Y4"
...
CSV_INCOME = OUTPUT_DIR / "COHR_quarterly_income.csv"
CSV_BALANCE = OUTPUT_DIR / "COHR_quarterly_balance.csv"
CSV_CASHFLOW = OUTPUT_DIR / "COHR_quarterly_cashflow.csv"
...
def fetch_cohr_quarterly() -> dict[str, pd.DataFrame]:
    t = yf.Ticker(TICKER)
    return {
...
def upload_google_sheets(statements: dict[str, pd.DataFrame]) -> None:
    ...
    sheet_slug = {
        "季損益 (Quarterly Income)": "COHR_QA_Income",
        "資產負債 (Quarterly Balance)": "COHR_QA_Balance",
        "現金流量 (Quarterly Cashflow)": "COHR_QA_Cashflow",
    }
...
def main() -> None:
    ...
    statements = fetch_cohr_quarterly()
