"""
NVDA dashboard — home: Stock Valuation (DCF + P/E zone). See pages/ for annual financials.
"""
...
def build_monthly_pe_history(ticker: str, years: int = 5) -> pd.DataFrame | None:
    ...
    # --- Path B: annual diluted EPS (covers ~5y when quarterly feed is short) ---
...
def pe_zone_message(
    ...
    if current_pe is None or current_pe <= 0 or not np.isfinite(current_pe):
        return (
            f"**{sym}** — No valid **trailing P/E** is available; cannot classify vs. history."
        )
    if hist_df is None or hist_df.empty or "pe" not in hist_df.columns:
        return (
            f"**{sym}** — Not enough historical monthly P/E to classify the current multiple."
        )
    s = hist_df["pe"].astype(float)
    s = s.replace([np.inf, -np.inf], np.nan).dropna()
    if len(s) < 6:
        return (
            f"**{sym}** — Too few monthly P/E observations to estimate stable percentiles."
        )
