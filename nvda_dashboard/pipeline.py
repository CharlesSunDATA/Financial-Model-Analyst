...
def _norm_stmt_label(label: str) -> str:
    """Yahoo / yfinance sometimes use 'Gross Profit' and sometimes 'GrossProfit' — compare without spaces/punct."""
...
def _union_quarter_ends(
    ...
    """
    Yahoo often returns fewer income-statement columns than CF/BS. Union all
    quarter-end dates so we can show extra rows with partial metrics (e.g. BS-only).
    If num_quarters is None, keep every distinct quarter Yahoo returns.
    """
...
def fetch_quarterly_metrics(ticker: str, num_quarters: int | None = None) -> pd.DataFrame:
    ...
    Income statement history is often shorter (~5 quarters) than cash flow or
    balance sheet; we merge **all quarter-end dates** from the three statements
    so extra periods appear with whatever line items exist (revenue may be blank).
    Pass num_quarters only to cap the count (optional).
    """
